...
func update_planned_intents() -> void:
	...
	if debuff_intent:
		...
		debuff_intent.current_text = "易伤"


func perform_action() -> void:
	if not enemy or not target:
		return
	_perform_strike_and_debuff(compute_damage_against_player(damage), _apply_vulnerable)
...
func _apply_vulnerable(targets: Array[Node]) -> void:
